...
if ( $_REQUEST[ '_logon' ] != $_SESSION[ $window ][ 'logon' ] )
{
   ...
   mymail( $json->mail->admin, 'security alert abhishektest', "possible security breach attempt on abhishektest\n" .
...
if ( $doc = $coll->findOne( array( "name" => $_SESSION[ $window ][ 'logon' ] ) ) )
{
        ...
        $_REQUEST[ 'changepassword' ] == "on" )
   {
      ...
      if ( $ok_to_update == 1 )
      {
         ...
         mymail( $doc[ 'email' ] , 'password change notice abhishektest', "Your password has been changed\n" .
        ...
        $_REQUEST[ 'changeemail' ] == "on" )
   {
      ...
      if ( $ok_to_update == 1 )
      {
         ...
         admin_mail( "[abhishektest][new email address] $email1", "User: " . $_REQUEST[ '_logon' ] . "\nEmail: $email1\n" );
