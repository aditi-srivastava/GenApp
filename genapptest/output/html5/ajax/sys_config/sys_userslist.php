...
function get_userinfo( $error_json_exit = false ) {
   ...
   $runs = $use_db->abhishektest->running->find();
   foreach ( $runs as $v ) {
       $uuid = $v['_id'];
       $job = $use_db->abhishektest->jobs->findOne( array( "_id" => $uuid ), array( "user" => 1 ) );
       $name = $job[ 'user' ];
   ...
   $users = $use_db->abhishektest->users->find();
   ...
   foreach ( $users as $v ) {
       ...
       $userinfo[] = 
           array( 
               ...
               ,"jobs-not-removed"   => $use_db->abhishektest->jobs->count( array( "user" => $name ) )
