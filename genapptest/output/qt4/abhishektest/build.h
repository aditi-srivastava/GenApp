void abhishektest::build_pressed()
{
   hide_widgets( menu_widgets );
   menu_button->setPixmap( id_to_icon[ "build" ] );
...

   {
      ...
      connect( pb, SIGNAL( clicked() ), SLOT( module_load_build_build_1() ) );
   ...
   }
   {
      ...
      pb->show();
      // this is supposed to automatically disconnect on deletion of the widget
      connect( pb, SIGNAL( clicked() ), SLOT( module_load_build_build_2() ) );
      if ( panel1_widgets.size() )
      {
         hbl->addSpacing( 2 );
      }
