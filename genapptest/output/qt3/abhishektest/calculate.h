void abhishektest::calculate_pressed()
{
   hide_widgets( menu_widgets );
   menu_button->setPixmap( id_to_icon[ "calculate" ] );
   ...
   {
      mQPushButton * pb = new mQPushButton( this );
      pb->setText( "Calculate 2" );
      pb->setMaximumHeight( 22 );
      pb->show();
      // this is supposed to automatically disconnect on deletion of the widget
      connect( pb, SIGNAL( clicked() ), SLOT( module_load_calculate_calculate_2() ) );
      if ( panel1_widgets.size() )
      {
         hbl->addSpacing( 2 );
      }
