...
abhishektest::abhishektest() : QFrame()
{
   setCaption( "SOME TITLE 0.1" );
...
void abhishektest::menu_pressed()
{
   hide_widgets( menu_widgets, menu_widgets[ 0 ]->isVisible() );
}

void abhishektest::hide_widgets( QValueVector < QWidget * > &widgets, bool hide )
{
...
void abhishektest::delete_widgets_layouts( QValueVector < QWidget * > &widgets, 
...
void abhishektest::save_value( const QString & id, const QString & ext )
{
   if ( global_data_types.count( id ) )
   {
      ...
      qDebug(
         QString( "Error: abhishektest::save_value( %1 ): unsupporded data type '%2'\n" )
         .arg( id )
   ...
   qDebug(
      QString( "Error: abhishektest::save_value( %1, %2 ): no global_data_type[] found\n" )
      .arg( id )
      .arg( ext )
          )
      ;
...
void abhishektest::save_last_value( const QString & id )
{
   save_value( id, "last_value" );
}

void abhishektest::save_default_value( const QString & id )
{
...
void abhishektest::save_last_values()
{
...
void abhishektest::reset_value( const QString & id, const QString & ext )
{
   if ( global_data_types.count( id ) )
   {
      QString did = id + ":" + ext;
      ...
      if ( global_data.count( did ) )
      {
         ...
         qDebug(
                QString( "Error: abhishektest::reset_value( %1, %2 ): unsupporded data type '%3'\n" )
                .arg( id )
                .arg( ext )
                .arg( global_data_types[ id ] )
                )
         ...
         qDebug( QString( "Error: abhishektest::reset value( %1, %2 ) no global data found\n" ).arg( id ).arg( ext) );
      }
      return;
   }
   qDebug(
      QString( "Error: abhishektest::reset_value( %1, %2 ): no global_data_type[] found\n" )
      .arg( id )
      .arg( ext )
          )
      ;
...
void abhishektest::reset_output_values( const QString & ext )
{
...
void abhishektest::reset_values( const QString & ext )
{
   for ( int i = 0; i < (int) panel1_inputs.size(); ++i )
   {
...
void abhishektest::reset_default_values()
{
   reset_values( "default_value" );
}

void abhishektest::reset_last_values()
{
   reset_values( "last_value" );
}

QString abhishektest::get_last_value( const QString & id, bool & skip )
{
   skip = false;
   if ( global_data_types.count( id ) )
   {
      ...
      qDebug(
         QString( "Error: abhishektest::get_last_value( %1 ): unsupporded data type '%2'\n" )
         .arg( id )
         .arg( global_data_types[ id ] )
             )
         ;
   ...
   qDebug(
      QString( "Error: abhishektest::get_last_value( %1 ): no global_data_type[] found\n" )
      .arg( id )
          )
      ;
   return "**error**";
...
QString abhishektest::input_to_json( const QString & mod, const QString & dir )
{
...
void abhishektest::process_results( const QString & mod )
{
         ...
         it++ )
   {
      ...
      if ( !global_data.count( key + ":default_value" ) )
      {
         ...
         if ( global_data_types.count( key ) )
         {
            ...
            if ( !ok )
            {
               qDebug( "abhishektest::process_results() unsupported output data type: " + global_data_types[ key ] + " key: " + key );
            }          
            //         global_data[ key + ":last_value" ] = QVariant( it.data() );
            ...
            qDebug( "abhishektest::process_results() global_data_types[] does not contain: " + key );
         }            
      }
   }
...
void abhishektest::push_back_color_if_ok( QColor bg, QColor set )
{
   double sum = 
...
void abhishektest::browse_filenames( const QString & label,
                                        const QString & id,
                                        bool  multiple_files )
{
...
void abhishektest::spawn_app( const QString & appname, const QString & filename )
{
